...
export default function UserPage() {
  ...
  const getSubCategories = () => {
    const normal =
      activeTab === "flower"
        ? ["all", "wedding", "shimigilina", "birthday", "anniversery", "graduation"]
        : activeTab === "decoration"
        ? ["all", "birthday", "shimigilina", "nika", "wedding", "babtaizm", "graduation"]
        : ["all", "men", "women", "children", "father", "mother", "new born"];
  ...
  const handleOrder = (p: Product, index: number) => {
    ...
    const phrases = translations[lang].orderPhrases as Record<string, string>;
    const phrase = phrases[p.type] || phrases.default;
      ...
      <main className="flex-grow p-4 md:p-6 lg:p-10 max-w-7xl mx-auto w-full">
          ...
          {["surprise", "flower", "decoration"].map(tab => (
            <button
              ...
              className={`px-6 py-2.5 rounded-full font-bold border-2 text-sm md:text-base ${
                activeTab === tab
                  ? "bg-[var(--brand-green)] text-white border-[var(--brand-green)] shadow-lg"
                  : "bg-[var(--brand-light)] text-[var(--brand-green)] border-[var(--brand-green)]"
              }`}
            >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
          {products.map((p, index) => {
            const title = p.type === "flower" ? "Flower" : "Package";

            return (
              <div
                key={p.id}
                className="bg-[var(--brand-light)] rounded-2xl md:rounded-3xl p-2 sm:p-3 md:p-4 shadow-sm border border-gray-100 flex flex-col h-full min-w-0 hover:shadow-md transition-shadow"
              >
                <div
                ...
                <div className="flex flex-col flex-1 pt-3 px-1">
                  <div>
                    ...
                    <p className="text-sm sm:text-base md:text-xl font-black text-[var(--brand-green)] mb-1.5">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="text-center md:text-left">
            ...
            <div className="flex justify-center md:justify-start gap-4">
              <a href="https://tiktok.com/@kido.surprise.delivery" target="_blank" className="w-9 h-9 rounded-full bg-gray-800 flex items-center justify-center">♪</a>
              <a href="https://instagram.com/kido122227" target="_blank" className="w-9 h-9 rounded-full bg-gray-800 flex items-center justify-center">◎</a>
              <a href="https://t.me/kidodelivery" target="_blank" className="w-9 h-9 rounded-full bg-gray-800 flex items-center justify-center">➤</a>
            </div>

            <p className="text-xs md:text-sm text-gray-300 mt-4">
              {translations[lang].footerContact}:{" "}
              <a href="tel:+251951161632" className="underline">
                +251 951 161 632
              </a>
            </p>
          </div>
          ...
          <div className="text-center md:text-right text-xs text-gray-400">
            <p className="mb-3">
              Developed by{" "}
              <a href="https://t.me/temesgenwalelign" target="_blank" className="text-[var(--brand-gold)] font-bold">
                Temesgen Walelgn
              </a>
              {" | "}
              <a href="tel:+251993370491">+251 993 370 491</a>
            </p>

            <p>
              © {new Date().getFullYear()} {translations[lang].brandName}.{" "}
