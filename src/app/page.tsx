    ...
    <div className="min-h-screen bg-[#f8f9fa] p-4 md:p-6 lg:p-10">
      ...
      <div className="flex flex-wrap justify-center gap-2 mb-8">
        {["surprise", "flower", "decoration"].map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`px-6 py-2.5 rounded-full font-bold transition-all duration-300 border-2 text-sm md:text-base ${activeTab === tab ? "bg-[#6ab04c] text-white border-[#6ab04c] shadow-lg scale-105" : "bg-white text-[#6ab04c] border-[#6ab04c] hover:bg-green-50"}`}>
            {tab.toUpperCase()}
          </button>
        ))}
      </div>
      ...
      <div className="flex justify-center flex-wrap gap-2 mb-8">
        {getSubCategories().map(sub => (
          <button key={sub} onClick={() => setActiveSub(sub)} className={`px-4 py-1.5 rounded-xl text-xs md:text-sm font-semibold transition-all ${activeSub === sub ? "bg-black text-white shadow-md" : "bg-white text-gray-500 hover:bg-gray-100"}`}>
            {sub.toUpperCase()}
          </button>
        ))}
      </div>
          ...
          <div key={p.id} className="bg-white rounded-3xl p-3 md:p-4 shadow-sm border border-gray-100 flex flex-row items-stretch gap-4 hover:shadow-md transition-shadow">
            ...
            <div className="w-1/2 flex flex-col justify-between py-1">
              <div>
                ...
                {p.flowerCount && <p className="text-[#6ab04c] text-xs font-bold mb-1">{p.flowerCount} Flowers</p>}
                <p className="text-sm md:text-lg font-black text-[#6ab04c] mb-2">{Number(p.price).toLocaleString()} ETB</p>
                <div className="text-[10px] md:text-xs text-gray-500 mb-2 leading-relaxed"><p className="line-clamp-3">{p.description}</p></div>
              </div>
              <button onClick={() => handleOrder(p)} className="w-full py-2 bg-gradient-to-r from-[#6ab04c] to-[#2e7d32] text-white text-xs md:text-sm font-bold rounded-xl hover:scale-[1.02] active:scale-[0.98] transition-transform shadow-sm">Order Now</button>
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex gap-2 justify-center">
        {product.images.map((img, i) => (
          <button key={i} onClick={() => setMain(img)} className={`w-16 h-16 rounded-lg overflow-hidden border-2 ${main === img ? "border-[#6ab04c]" : "border-transparent"}`}>
